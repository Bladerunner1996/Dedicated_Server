...
void AMatchPlayerState::OnMatchEnded(const FString& Username)
{
	...
	//AMatchGameState* MatchGameState = GetWorld()->GetGameState<AMatchGameState>();
	...
	RecordMatchStats(RecordMatchStatsInput);  // 父类的protected函数，调用 GameStatsManager 发送HTTP请求，（把 RecordMatchStatsInput struct转化为 json string）
}
